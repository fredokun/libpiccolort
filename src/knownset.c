...
PICC_KnownSet *PICC_create_knownset(int init_max_size, PICC_Error* e){
...
void PICC_free_knownset(PICC_KnownSet *s)
{
    free(s->content);
    free(s);
}

bool PICC_knownset_add(PICC_KnownSet *s, PICC_KnownValue *val)
{
...
PICC_KnownSet *PICC_knownset_known(PICC_KnownSet *ks){
    PICC_KnownSet* s2 = PICC_create_empty_knownset();

    PICC_KnownElement *e;
    PICC_KNOWNSET_FOREACH_ELEM(ks, e){
	if(e->state == PICC_KNOWN)
	    PICC_knownset_add(s2, e->value);
    }
